...
nodes_config = [
    ...
    (
        "parse_metadata",
        "completed",
        "Successfully parsed: 5 authors, 23 references, 12 keywords",
        None,
    ),
    ...
    (
        "extract_tables",
        "completed",
        "Extracted and processed 4 tables with 156 total rows",
        None,
    ),
    (
        "nlp_processing",
        "failed",
        "NLP model timeout after 300s\nTraceback: ConnectionError at line 45",
        "ConnectionError: Model inference server unreachable",
    ),
...
for node_id, status, output_msg, error_msg in nodes_config:
    node_def = next(
        n for n in workflow_def.dag_structure["nodes"] if n["id"] == node_id
    )
    ...
    node = WorkflowNode.objects.create(
        ...
        claimed_at=(
            timezone.now() if status in ["running", "completed", "failed"] else None
        ),
        started_at=(
            timezone.now() if status in ["running", "completed", "failed"] else None
        ),
