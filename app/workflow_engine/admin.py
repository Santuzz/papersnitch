...
class WorkflowRunAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        ("Input/Output", {"fields": ["input_data", "output_data"]}),
        ...
        ("Highlighted PDF", {"fields": ["highlighted_pdf_link"]}),
    ...
    def paper_link(self, obj):
        try:
            ...
            pass
        return format_html('<span style="color: #999;">[Deleted Paper]</span>')
    ...
    def highlighted_pdf_link(self, obj):
        """Display link to highlighted PDF or generation button."""
        # Look for highlighted_pdf artifact in any of the workflow run's nodes
        from .models import NodeArtifact

        artifact = NodeArtifact.objects.filter(
            node__workflow_run=obj, name="highlighted_pdf", artifact_type="file"
        ).first()

        if artifact and artifact.file:
            file_url = artifact.file.url
            file_size_mb = (
                artifact.size_bytes / (1024 * 1024) if artifact.size_bytes else 0
            )

            return format_html(
                '<a href="{}" target="_blank" style="display: inline-block; padding: 8px 12px; background-color: #4CAF50; color: white; text-decoration: none; border-radius: 4px;">'
                '<i class="fas fa-file-pdf"></i> View Highlighted PDF'
                "</a> "
                '<span style="color: #666; margin-left: 10px;">({:.2f} MB)</span>',
                file_url,
                file_size_mb,
            )
        else:
            # Provide link to generate the PDF
            generate_url = f"/workflow/{obj.id}/highlighted-pdf/"
            return format_html(
                '<span style="color: #999;">Not generated yet</span> '
                '<a href="{}" target="_blank" style="display: inline-block; margin-left: 10px; padding: 6px 10px; background-color: #2196F3; color: white; text-decoration: none; border-radius: 4px; font-size: 12px;">'
                "Generate Now"
                "</a>",
                generate_url,
            )

    highlighted_pdf_link.short_description = "Highlighted PDF"
...
class WorkflowNodeAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ...
        (
            "Error Information",
            {"fields": ["error_message", "error_traceback"], "classes": ["collapse"]},
        ),
        (
            "Timestamps",
            {
