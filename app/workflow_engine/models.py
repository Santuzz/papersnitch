...
class WorkflowDefinition(models.Model):
    ...
    version = models.IntegerField(
        default=1, help_text="Version number for workflow evolution"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["name", "version"], name="unique_workflow_name_version"
            )
        ]
...
class WorkflowRun(models.Model):
    ...
    workflow_definition = models.ForeignKey(
        WorkflowDefinition, on_delete=models.PROTECT, related_name="runs"
    )
    ...
    run_number = models.IntegerField(
        default=1, help_text="Sequential run number for this paper (1, 2, 3...)"
    )

    # Context data passed to the workflow
    input_data = models.JSONField(
        default=dict, help_text="Input parameters for this workflow run"
    )

    # Results and error tracking
    output_data = models.JSONField(
        default=dict, blank=True, help_text="Final output/results from the workflow"
    )
    error_message = models.TextField(blank=True, null=True)

    # Token usage tracking (aggregated from all nodes)
    total_input_tokens = models.IntegerField(
        default=0,
        help_text="Total input tokens consumed across all nodes in this workflow run",
    )
    total_output_tokens = models.IntegerField(
        default=0,
        help_text="Total output tokens generated across all nodes in this workflow run",
    )
    total_tokens = models.IntegerField(
        default=0, help_text="Total tokens (input + output) used by this workflow run"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["paper", "total_tokens"]),
        ]
    ...
    def __str__(self):
        try:
            paper_title = self.paper.title[:50] if self.paper else "[No Paper]"
        except:
            paper_title = "[Deleted Paper]"
        return f"{self.workflow_definition.name} - Run #{self.run_number} for {paper_title}"

    def save(self, *args, **kwargs):
        """Auto-increment run_number for the same paper."""
        if not self.run_number:
            last_run = (
                WorkflowRun.objects.filter(paper=self.paper)
                .order_by("-run_number")
                .first()
            )
    ...
    def get_progress(self) -> Dict[str, Any]:
        """Calculate workflow progress statistics in memory to avoid DB hits.

        Skipped nodes are excluded from the total so they don't distort the
        percentage or completion count.  The workflow status is also unaffected
        by skipped nodes (a run is only failed when there are actual failed nodes).
        """
        # Grab all nodes into a Python list (uses prefetched data if available)
        all_nodes = list(self.nodes.all())

        # Calculate counts in memory (blazing fast)
        skipped = sum(1 for n in all_nodes if n.status in ["skipped", "cancelled"])
        ...
        pending = sum(1 for n in all_nodes if n.status in ["pending", "ready"])

        # Total excludes skipped/cancelled nodes so progress reflects only
        # the nodes that are actually expected to run.
        total = len(all_nodes) - skipped
        ...
        if total == 0:
            return {
                ...
                "pending": 0,
                "skipped": skipped,
                "percentage": 100 if skipped > 0 else 0,
            }
...
class WorkflowNode(models.Model):
    ...
    workflow_run = models.ForeignKey(
        WorkflowRun, on_delete=models.CASCADE, related_name="nodes"
    )
    ...
    node_type = models.CharField(
        max_length=100, help_text="Type of task (e.g., 'celery', 'langgraph', 'python')"
    )
    ...
    claim_expires_at = models.DateTimeField(
        null=True, blank=True, help_text="Claim expiration for stale worker detection"
    )
    ...
    # Input/output data
    input_data = models.JSONField(
        default=dict, help_text="Input parameters for this node"
    )
    output_data = models.JSONField(
        default=dict, blank=True, help_text="Output/results from this node"
    )
    ...
    input_tokens = models.IntegerField(
        default=0, help_text="Number of input tokens consumed by this node"
    )
    output_tokens = models.IntegerField(
        default=0, help_text="Number of output tokens generated by this node"
    )
    total_tokens = models.IntegerField(
        default=0, help_text="Total tokens (input + output) used by this node"
    )
    was_cached = models.BooleanField(
        default=False,
        help_text="Whether this node reused cached results and copied token counts from previous execution",
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["workflow_run", "was_cached"]),
        ]
    ...
    def get_dependencies(self) -> List["WorkflowNode"]:
        ...
        return WorkflowNode.objects.filter(
            workflow_run=self.workflow_run, node_id__in=dep_ids
        )
    ...
    def get_dependents(self) -> List["WorkflowNode"]:
        ...
        return WorkflowNode.objects.filter(
            workflow_run=self.workflow_run, node_id__in=dependent_ids
        )
...
class NodeArtifact(models.Model):
    ...
    node = models.ForeignKey(
        WorkflowNode, on_delete=models.CASCADE, related_name="artifacts"
    )
    ...
    file_path = models.CharField(
        max_length=500, blank=True, null=True, help_text="Path to file in media storage"
    )
    file = models.FileField(
        upload_to="workflow_artifacts/",
        blank=True,
        null=True,
        help_text="File upload for artifacts (e.g., highlighted PDFs, reports)",
    )
    url = models.URLField(max_length=1000, blank=True, null=True)
    inline_data = models.JSONField(
        blank=True, null=True, help_text="Small data stored inline"
    )
    ...
    metadata = models.JSONField(
        default=dict, blank=True, help_text="Additional metadata about the artifact"
    )
...
class NodeLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    node = models.ForeignKey(
        WorkflowNode, on_delete=models.CASCADE, related_name="logs"
    )

    level = models.CharField(max_length=10, choices=LOG_LEVEL_CHOICES, default="INFO")
    message = models.TextField()
    context = models.JSONField(
        default=dict, blank=True, help_text="Additional context data for the log entry"
    )
