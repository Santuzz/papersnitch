...
# Add app to path
STACK_SUFFIX = os.getenv("STACK_SUFFIX", "dev")
...
class ScrapingStatusView(View):
    ...
    def get(self, request, conference_id):
        ...
        data = {
            ...
            "last_scrape_start": (
                conference.last_scrape_start.isoformat()
                if conference.last_scrape_start
                else None
            ),
            "last_scrape_end": (
                conference.last_scrape_end.isoformat()
                if conference.last_scrape_end
                else None
            ),
            "log_file": (
                os.path.basename(conference.scraping_log_file)
...
class StopScrapingView(UserPassesTestMixin, View):
    ...
    def post(self, request, conference_id):
        ...
        if not conference.is_scraping:
            return JsonResponse(
                {"success": False, "message": "No scraping is currently in progress."},
                status=400,
            )
        ...
        return JsonResponse({"success": True, "message": "Scraping has been stopped."})
...
class CreateAndScrapeConferenceView(UserPassesTestMixin, View):
    ...
    def post(self, request):
        ...
        if not papers_url:
            return JsonResponse(
                {"success": False, "message": "Papers URL is required."}, status=400
            )
        ...
        if year:
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": "Year must be a valid number."},
                    status=400,
                )
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"Error creating conference: {str(e)}"},
                status=500,
            )
