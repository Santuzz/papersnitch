"""
Django management command to initialize dataset documentation criteria embeddings.
...
from webApp.models import DatasetDocumentationCriterion
from webApp.services.nodes.dataset_documentation_criteria import get_all_criteria

logger = logging.getLogger(__name__)


class Command(BaseCommand):
    help = "Initialize dataset documentation criteria with embeddings"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Initializing dataset documentation criteria embeddings")
        ...
        for criterion in criteria:
            try:
                ...
                exists = DatasetDocumentationCriterion.objects.filter(
                ...
                with transaction.atomic():
                    if exists:
                        # Update existing
                        obj = DatasetDocumentationCriterion.objects.get(
                            criterion_id=criterion.criterion_id,
                            embedding_model=embedding_model
                        )
                        ...
                        # Create new
                        DatasetDocumentationCriterion.objects.create(
                            criterion_id=criterion.criterion_id,
        ...
        total_in_db = DatasetDocumentationCriterion.objects.filter(
            embedding_model=embedding_model
        ).count()
        ...
        for category in ['data_collection', 'annotation', 'ethics_availability']:
            count = DatasetDocumentationCriterion.objects.filter(
                embedding_model=embedding_model,
                category=category
            ).count()
