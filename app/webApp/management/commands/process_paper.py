...
from webApp.services.paper_processing_workflow import process_paper_workflow, process_multiple_papers
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if paper_id:
            ...
            try:
                ...
                raise CommandError('Invalid batch format. Use comma-separated integers: 1,2,3')
        else:  # process_all
        ...
        if missing_ids:
            self.stdout.write(self.style.ERROR(
                f'Papers not found: {sorted(missing_ids)}'
            ))
        ...
        self.stdout.write(self.style.SUCCESS(
            f'Processing {len(paper_ids)} paper(s)...'
        ))
        if force:
    ...
    def _show_history(self, paper_id):
        ...
        if not runs:
            self.stdout.write(self.style.WARNING(f'No workflow runs found for paper: {paper.title}'))
            return
        ...
        for run in runs:
            ...
            self.stdout.write(f'Created: {run.created_at.strftime("%Y-%m-%d %H:%M:%S")}')
            if run.completed_at:
                duration = (run.completed_at - run.started_at).total_seconds() if run.started_at else None
                self.stdout.write(f'Completed: {run.completed_at.strftime("%Y-%m-%d %H:%M:%S")} ({duration:.1f}s)' if duration else f'Completed: {run.completed_at.strftime("%Y-%m-%d %H:%M:%S")}')

            # Show nodes
            nodes = WorkflowNode.objects.filter(workflow_run=run)
            for node in nodes:
                status_color = self.style.SUCCESS if node.status == 'completed' else (
                    self.style.ERROR if node.status == 'failed' else self.style.WARNING
                )
                self.stdout.write(f'  Node: {node.node_id} - {status_color(node.status.upper())}')
                
                # Show main artifact
                result_artifact = NodeArtifact.objects.filter(node=node, name='result').first()
                if result_artifact and result_artifact.inline_data:
                    if node.node_id == 'paper_type_classification':
                        self.stdout.write(f'    Paper Type: {result_artifact.inline_data.get("paper_type", "unknown")}')
                        self.stdout.write(f'    Confidence: {result_artifact.inline_data.get("confidence", 0):.2f}')
                    elif node.node_id == 'code_reproducibility_analysis':
                        code_avail = result_artifact.inline_data.get('code_availability', {})
                        self.stdout.write(f'    Code Available: {code_avail.get("code_available", False)}')
    ...
    def _display_result(self, result):
        ...
        if result.get('success'):
            ...
            if code_analysis:
                ...
                self.stdout.write(f"Code Available: {code_analysis.code_availability.code_available}")
                if code_analysis.code_availability.code_url:
                    self.stdout.write(f"Code URL: {code_analysis.code_availability.code_url}")
            ...
            self.stdout.write(f"Total tokens: {result['total_input_tokens'] + result['total_output_tokens']}")
        ...
        if failed > 0:
            ...
            for result in results:
                if isinstance(result, Exception):
                    ...
                    self.stdout.write(self.style.ERROR(f'  - Paper {paper_id}: {error}'))
