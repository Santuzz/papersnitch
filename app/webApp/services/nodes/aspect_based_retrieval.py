...
async def get_or_create_aspect_embedding(
    aspect_id: str, client: OpenAI, model: str = "text-embedding-3-small"
) -> ReproducibilityAspectEmbedding:
    """
    ...
    # Try to get existing
    try:
        aspect_emb = await sync_to_async(ReproducibilityAspectEmbedding.objects.get)(
    ...
    # Generate embedding
    response = client.embeddings.create(model=model, input=context_text)

    embedding_vector = response.data[0].embedding
    dimension = len(embedding_vector)

    # Save to database
    aspect_emb = await sync_to_async(ReproducibilityAspectEmbedding.objects.create)(
        aspect_id=aspect_id,
...
async def retrieve_relevant_code(
    ...
    if not code_files:
        logger.warning(
            f"No code embeddings found for paper {paper_id}, repo {code_url}"
        )
        return []

    logger.info(
        f"Found {len(code_files)} code embeddings for paper {paper_id}, repo {code_url}"
    )
    ...
    for code_file in code_files:
        similarity = code_file.compute_cosine_similarity(aspect_embedding.embedding)
...
async def analyze_aspect(
    ...
    if relevant_sections:
        ...
        if node:
            await async_ops.create_node_log(
                node, "DEBUG", f"Top sections:\n{section_details}"
            )
    else:
        logger.warning(f"Aspect {aspect_id}: No sections retrieved!")
        if node:
            await async_ops.create_node_log(
                node, "WARNING", "No sections retrieved - analysis may be incomplete"
            )
    ...
    if relevant_code:
        ...
        if node:
            await async_ops.create_node_log(
                node, "DEBUG", f"Top code files:\n{code_details}"
            )
    else:
        logger.warning(f"Aspect {aspect_id}: No code files retrieved!")
        if node:
            await async_ops.create_node_log(
                node, "WARNING", "No code files retrieved - analysis may be incomplete"
            )
    ...
    prompt = aspect_def.analysis_prompt_template.format(
        sections_text=sections_text, code_text=code_text
    )
    ...
    logger.info(
        f"Aspect {aspect_id}: Prompt length: {len(prompt):,} chars (~{prompt_tokens:,} tokens)"
    )
    if node:
        await async_ops.create_node_log(
            node,
    ...
    sections_preview = (
        sections_text[:200] + "..." if len(sections_text) > 200 else sections_text
    )
    ...
    # Call LLM
    logger.info(
        f"Calling LLM for aspect {aspect_id} (estimated prompt: {prompt_tokens:,} tokens)..."
    )
    if node:
        await async_ops.create_node_log(
            node,
    ...
    logger.info(
        f"Aspect {aspect_id} analysis complete (actual: {input_tokens:,} input, {output_tokens:,} output tokens)"
    )
    if node:
        await async_ops.create_node_log(
            node,
            "INFO",
            f"Aspect {aspect_id} analysis complete ({input_tokens:,} input, {output_tokens:,} output tokens)",
        )
