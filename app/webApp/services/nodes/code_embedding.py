...
async def compute_embedding(
    client, text: str, model: str = "text-embedding-3-small"
...
async def code_embedding_node(state: PaperProcessingState) -> Dict[str, Any]:
    ...
    logger.info(f"Node F: Starting code embedding for paper {state['paper_id']}")
    ...
    await async_ops.create_node_log(node, "INFO", "Starting code repository embedding")
    ...
    try:
        ...
        if not code_availability or not code_availability.code_available:
            ...
            return {"code_embedding_result": None, "total_files": 0, "total_chunks": 0}
        ...
        await async_ops.create_node_log(
            node,
            "INFO",
            f"Getting documentation and repository structure from: {source}",
        )
...
"""
        ...
        logger.info(
            f"LLM suggested code file patterns: {retrieved_patterns.included_patterns}"
        )
        await async_ops.create_node_log(
            node,
            "INFO",
            ...
            for chunk_index, chunk_content in enumerate(chunks):
                ...
                try:
                    ...
                    embedding_dimension = len(
                        embedding
                    )  # Get dimension from actual embedding
                    ...
                    logger.error(
                        f"Error embedding {file_path} chunk {chunk_index}: {e}"
                    )
                    await async_ops.create_node_log(
                        node,
                        "WARNING",
        ...
        embedding_dimension = (
            len(embedded_files[0].embedding) if embedded_files else 1536
        )
        ...
        # Create artifact version without embedded_files details (already stored in DB)
        # This reduces artifact size significantly and avoids cluttering the webpage display
        artifact_result = result.model_copy()
        artifact_result.embedded_files = []  # Clear embedded files list (data is in DB)

        # Store as artifacts (without embedded file details to reduce size)
        await async_ops.create_node_artifact(node, "result", artifact_result)
        await async_ops.create_node_artifact(
            node,
            "token_usage",
            {
