...
async def reproducibility_checklist_node(
    ...
    await async_ops.create_node_log(
        node,
        "INFO",
        "Starting multi-step reproducibility analysis (20 criteria, excludes code)",
    )
    ...
    try:
        ...
        if not force_reprocess:
            ...
            if previous_node:
                ...
                if previous_artifact and previous_artifact.inline_data:
                    ...
                    result = AggregatedReproducibilityAnalysis(
        ...
        criteria_models = await sync_to_async(
            lambda: list(
                ReproducibilityChecklistCriterion.objects.filter(
        ...
        for i, criterion_model in enumerate(criteria_models, 1):
            logger.info(f"Analyzing criterion {i}/20: {criterion_model.criterion_name}")
...
6. Importance level for THIS {paper_type} paper: 'critical', 'important', or 'optional'"""
            ...
            try:
                ...
                logger.error(
                    f"  Error analyzing criterion {criterion_model.criterion_name}: {e}"
                )
                await async_ops.create_node_log(
                    node,
                    "WARNING",
                    f"Failed criterion {criterion_model.criterion_name}: {str(e)}",
                )
                # Continue with other criteria
                continue

        logger.info(
            f"Completed individual criterion analyses: {len(criterion_analyses)}/20"
        )
        ...
        await async_ops.create_node_artifact(
            node, "criterion_analyses", [c.model_dump() for c in criterion_analyses]
        )
...
def _generate_programmatic_assessment(
    ...
    score_interpretation = (
        ...
        if weighted_score >= 80
            ...
            else "moderate" if weighted_score >= 40 else "limited"
        )
    )
    ...
    if len(critical_gaps) > 0:
        summary += (
            f"{len(critical_gaps)} critical reproducibility criteria are missing."
        )
    else:
    ...
    # Extract strengths
    strengths = []
    for analysis in sorted(present_criteria, key=lambda a: a.confidence, reverse=True)[
        :7
    ]:
        if analysis.evidence_text:
            evidence_preview = (
                analysis.evidence_text[:80] + "..."
                if len(analysis.evidence_text) > 80
                else analysis.evidence_text
            )
            strengths.append(f"{analysis.criterion_name}: {evidence_preview}")
        else:
    ...
    for analysis in sorted(
        absent_criteria, key=lambda a: 0 if a.importance == "important" else 1
    )[: 7 - len(weaknesses)]:
        if analysis.notes:
    ...
    for analysis in absent_criteria:
        if analysis.importance == "important" and len(recommendations) < 7:
            recommendations.append(f"Consider documenting {analysis.criterion_name}")
...
async def _aggregate_criterion_analyses(
    criterion_analyses: List[SingleCriterionAnalysis], paper_type: str, paper_title: str
) -> AggregatedReproducibilityAnalysis:
    """
    ...
    logger.info(
        "Generating assessment text programmatically (no LLM aggregation call)..."
    )
    ...
    logger.info(
        f"Programmatic assessment complete: {len(strengths)} strengths, {len(weaknesses)} weaknesses, {len(recommendations)} recommendations"
    )
