...
async def final_aggregation_node(
    ...
    logger.info(
        f"Dependencies in state: repro_checklist={repro_checklist is not None}, "
    ...
    if not repro_checklist:
        logger.info(
            "final_aggregation invoked too early - reproducibility_checklist not yet complete. Returning early."
        )
        return {}
    ...
    if needs_dataset_docs and not dataset_docs_result:
        logger.info(
            "final_aggregation invoked too early - dataset_documentation_check not yet complete. Returning early."
        )
        return {}
    ...
    logger.info(
        f"Needs code analysis: {needs_code_analysis}, Has code analysis: {code_analysis is not None}"
    )

    # Check code_repository_analysis if required
    if needs_code_analysis and not code_analysis:
        logger.info(
            "final_aggregation invoked too early - code_repository_analysis not yet complete. Returning early."
        )
        return {}

    # All dependencies ready - proceed with execution
    logger.info(
        f"All dependencies ready. Proceeding with final_aggregation for paper {state['paper_id']}"
    )

    # Check if already completed (idempotency guard: LangGraph calls this node once per
    # incoming edge, so it can be invoked again after we already finished via the cache path)
    if node.status == "completed":
        logger.warning("final_aggregation already completed - returning cached result")
        # The result may already be in the state (set by the first invocation)
        existing_result = state.get("final_assessment_result")
        if existing_result:
            return {"final_assessment_result": existing_result}
        # Fall back to reading from the node artifact
        result_artifact = await async_ops.get_node_artifact(node, "result")
        if result_artifact and result_artifact.inline_data:
            return {
                "final_assessment_result": FinalReproducibilityAssessment(
                    **result_artifact.inline_data
                )
            }
    ...
    await async_ops.create_node_log(
        node,
        "INFO",
        "Aggregating findings from reproducibility checklist, code analysis, and dataset documentation",
    )
    ...
    try:
        ...
        logger.info(
            f"Inputs: repro_checklist={repro_checklist is not None}, "
            f"code_analysis={code_analysis is not None}, "
        ...
        if not force_reprocess:
            ...
            if previous_node:
                ...
                if previous_artifact and previous_artifact.inline_data:
                    logger.info(
                        f"Found previous aggregation from {previous_node.completed_at}"
                    )
                    await async_ops.create_node_log(
                        node,
                        "INFO",
                    ...
                    result = FinalReproducibilityAssessment(
        ...
        analyses_summary.append(
            f"- Models/Algorithms: {repro_checklist.models_score:.1f}/100"
        )
        analyses_summary.append(f"- Datasets: {repro_checklist.datasets_score:.1f}/100")
        analyses_summary.append(
            f"- Experiments: {repro_checklist.experiments_score:.1f}/100"
        )
        analyses_summary.append(f"- Overall: {repro_checklist.overall_score:.1f}/100")
        analyses_summary.append(
            f"- Weighted ({paper_type}): {repro_checklist.weighted_score:.1f}/100"
        )
        ...
        analyses_summary.append(
            f"- Weaknesses: {'; '.join(repro_checklist.weaknesses)}"
        )
        ...
        if has_code_analysis:
            ...
            analyses_summary.append(
                f"- Conde reproducibility Score: {code_analysis.reproducibility_score:.1f}/100"
            )
            code_score_normalized = (
                code_analysis.reproducibility_score
            )  # Already 0-100 scale
            analyses_summary.append(
                f"- Score Breakdown: {', '.join(f'{k}: {v:.1f}' for k, v in code_analysis.score_breakdown.items())}"
            )
            analyses_summary.append(
                f"- Assessment: {code_analysis.overall_assessment[:300]}"
            )
            if code_analysis.recommendations:
                analyses_summary.append(
                    f"- Recommendations: {'; '.join(code_analysis.recommendations[:3])}"
                )
        else:
            analyses_summary.append(
        ...
        if has_dataset_analysis:
            ...
            analyses_summary.append(
                f"- News Dataset Score: {dataset_docs.overall_score:.1f}/100"
            )
        ...
        if abs(total_weight - 1.0) > 0.001:
            logger.warning(
                f"Component weights don't sum to 1.0 (sum={total_weight}), normalizing"
            )
        ...
        await async_ops.create_node_log(
            node,
            "INFO",
            "Generating qualitative assessment (summary, strengths, weaknesses)",
        )
...
Create a unified narrative connecting findings across all evaluation dimensions."""
        ...
        repro_checklist_node = await async_ops.get_workflow_node(
            state["workflow_run_id"], "reproducibility_checklist"
        )
        criterion_analyses_artifact = await async_ops.get_node_artifact(
            repro_checklist_node, "criterion_analyses"
        )
        ...
        #     "present": false,
        #     "confidence": 0.63,
        #     "evidence_text": "\"For a model vertex P_i, its position after deformation is calculated using its m neighboring nodes:\" (formula not shown). \"Specifically, the curves C_k and C̄_k are transformed to their SRVF representations, q_{C_k} and q_{C̄_k}. Then, the alignment problem is formulated as estimating the optimal rotation O ∈ SO(2) and reparameterization func... [truncated]\"",
        #     "page_reference": "Methodology; Problem Description",
        #     "notes": "While the text mentions ED graph with affine transforms and SRVF-based registration and references projection model with K, R_k, t_k, the explicit equations are missing or truncated. Key formulas for deformation interpolation, SRVF definition q(t)=c'(t)/sqrt(||c'(t)||), the optimization objective, weights, regularizers, and computational steps are not fully provided in the excerpts. Therefore, a full mathematical description cannot be confirmed from provided sections.",
        #     "importance": "critical"
        #   },]}}
        ...
        evaluation_details["paper_checklist"] = {
            ...
            # "criteria": individual_criteria["value"],
        ...
        if has_code_analysis:
            evaluation_details["code_analysis"] = {
                "research_methodology": (
                    code_analysis.research_methodology.model_dump()
                    if code_analysis.research_methodology
                    else None
                ),
                "repository_structure": (
                    code_analysis.repository_structure.model_dump()
                    if code_analysis.repository_structure
                    else None
                ),
                "code_components": (
                    code_analysis.code_components.model_dump()
                    if code_analysis.code_components
                    else None
                ),
                "artifacts": (
                    code_analysis.artifacts.model_dump()
                    if code_analysis.artifacts
                    else None
                ),
                "dataset_splits": (
                    code_analysis.dataset_splits.model_dump()
                    if code_analysis.dataset_splits
                    else None
                ),
                "documentation": (
                    code_analysis.documentation.model_dump()
                    if code_analysis.documentation
                    else None
                ),
        ...
        if has_dataset_analysis:
            dataset_docs_node = await async_ops.get_workflow_node(
                state["workflow_run_id"], "dataset_documentation_check"
            )
            dataset_criterion_analyses_artifact = await async_ops.get_node_artifact(
                dataset_docs_node, "criterion_analyses"
            )

            if (
                dataset_criterion_analyses_artifact
                and dataset_criterion_analyses_artifact.inline_data
            ):
                dataset_individual_criteria = (
                    dataset_criterion_analyses_artifact.inline_data
                )
            else:
                logger.warning(
                    "No dataset criterion_analyses artifact found - using empty list"
                )
                dataset_individual_criteria = []
        ...
        logger.info(
            f"Merged evaluation details with {len([k for k, v in evaluation_details.items() if v is not None])} components"
        )
        ...
        final_result = FinalReproducibilityAssessment(
            ...
            dataset_documentation_score=(
                dataset_docs.overall_score if has_dataset_analysis else None
            ),
        ...
        await async_ops.update_node_status(node, "failed", completed_at=timezone.now())
        raise
