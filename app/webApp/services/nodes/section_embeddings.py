...
async def section_embeddings_node(state: PaperProcessingState) -> Dict[str, Any]:
    ...
    await async_ops.create_node_log(
        node, "INFO", "Starting section embeddings computation"
    )
    ...
    try:
        ...
        has_sections = (
            paper.sections
            and isinstance(paper.sections, dict)
            and len(paper.sections) > 0
        )
        has_text = paper.text and len(paper.text.strip()) > 0

        if not has_sections and not has_text:
            logger.warning(
                f"Paper {state['paper_id']} has no sections or text, skipping embeddings"
            )
            await async_ops.create_node_log(
            ...
            return {
                "section_embeddings_result": {"sections_processed": 0, "skipped": True}
            }
        ...
        if paper.sections and isinstance(paper.sections, dict):
            ...
            logger.info(
                f"Using {len(paper.sections)} sections from Paper.sections field"
            )
            ...
            for section_name, section_text in paper.sections.items():
                if section_text and len(str(section_text).strip()) > 100:
                    ...
                    normalized_name = (
                        section_name.lower().replace(" ", "_").replace("-", "_")
                    )
            ...
            logger.warning(
                f"Paper {state['paper_id']} has no sections field, falling back to extraction"
            )
        ...
        for section_type, section_text in sections:
            try:
                ...
                embedding = await compute_embedding(
                    client, section_text, embedding_model
                )
                ...
                # Store in database
                await sync_to_async(PaperSectionEmbedding.objects.update_or_create)(
                    paper_id=state["paper_id"],
                ...
                logger.info(
                    f"Stored embedding for {section_type} ({len(embedding)} dimensions)"
                )
        ...
        await async_ops.update_node_tokens(
            node, input_tokens=total_tokens, output_tokens=0, was_cached=False
        )
        ...
        if paper_type_result:
            ...
            if paper_type_result.paper_type == "theoretical":
                ...
                for node_id in skipped_nodes:
                    skip_node = await async_ops.get_workflow_node(
                    ...
                    if skip_node and skip_node.status == "pending":
                        ...
                        await async_ops.create_node_log(
                            skip_node,
                            "INFO",
                            "Skipped (theoretical paper - no code/dataset analysis)",
                        )
