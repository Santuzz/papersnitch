...
async def dataset_documentation_check_node(
    ...
    try:
        ...
        criteria_models = await sync_to_async(
            lambda: list(
                DatasetDocumentationCriterion.objects.filter(
        ...
        for i, criterion_model in enumerate(criteria_models, 1):
            logger.info(f"Analyzing criterion {i}/10: {criterion_model.criterion_name}")
...
6. Importance level for dataset papers: 'critical', 'important', or 'optional'"""
        ...
        logger.info(
            f"Completed individual criterion analyses: {len(criterion_analyses)}/10"
        )
        ...
        await async_ops.create_node_log(
            node,
            "INFO",
            f"Aggregating {len(criterion_analyses)} criterion analyses programmatically",
        )
...
def _extract_dataset_metadata(
    ...
    logger.debug(
        f"Extracted metadata: name={dataset_name}, size={dataset_size}, link={download_link}"
    )
...
def _generate_programmatic_assessment(
    ...
    score_interpretation = (
        ...
        if overall_score >= 80
            ...
            "good"
            if overall_score >= 60
            else "moderate" if overall_score >= 40 else "limited"
        )
    )
